...
commands:
  ...
  down <project>       stop
  logs <project> [process] [-f]
                       show logs (-f opens live view)
  ...
  quit / exit          leave the repl"
        ...
        "logs" => {
            ...
            let project = rest
                .first()
                .copied()
                .ok_or_else(|| anyhow::anyhow!("usage: logs <project> [process] [-f]"))?;
            match rest.get(1) {
                Some(process) => {
                    let path = project_path(project)?;
                    crate::logs::print_process_logs(&path, process, follow)?;
                }
                None if follow => {
                    let _ = run_live_view(project, false)?;
                }
                None => crate::logs::print_project_logs(project, false)?,
            }
            Ok(ReplAction::Continue)
        }
        "bash" => {
